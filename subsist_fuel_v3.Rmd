...
```{r}
posterior <- as.matrix(Mglm)

#main hh_level predictors
hh_posterior_fig <- mcmc_areas(posterior, pars = c( "FishSumEdible1000", "LargeGameSumEdible1000", "sms","boats","atvs","cars","(Intercept)"),prob = 0.8) +   ggplot2::scale_y_discrete(labels = c( "Fish harvest 1000lbs", "Game harvest 1000lbs","Snowmobiles","Boats","ATVs", "Cars & trucks","Intercept")) + 
...
#gasoline price predictor by community
...
PPC <- ppc_dens_overlay(y = Mglm$y, yrep = posterior_predict(Mglm, draws = 50))+
  ggplot2::ylab("Density")+
  ggplot2::xlab("Log(annual gasoline consumption)")
PPC + theme_update(text = element_text(size=7))+
...
library(gginnards)
library(stringr)
draws <- as.data.frame(t(posterior_predict(Mglm, draws = 50)))
comPredict <- cbind(Mglm$y,draws,linearPredict)

library(reshape2)
data<- melt(comPredict)
data$type <- with(data, ifelse(str_detect(data$variable,"V"),"y HB",ifelse(variable=="Mglm$y","y","y ML")))
...
compare_models(kfoldHB1, kfoldHB2, detail=TRUE) #negative 'elpd_diff' favors 1st model, positive favors 2nd
```
Result:
Model comparison: 
...
kfoldHB2 <- kfold(Mglm2, K=10)
compare_models(kfoldHB3, kfoldHB2, detail=TRUE) #negative 'elpd_diff' favors 1st model, positive favors 2nd
```

using loo cv
```{r}
library(loo)
loo1 <- loo(Mglm, cores = 2)
print(loo1)

loo2 <- loo(Mglm2, cores = 2)
print(loo2)
