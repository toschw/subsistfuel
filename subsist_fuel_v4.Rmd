...
ggplot(data = a_df_exp2, 
       aes(x = commname, 
           y = a_mean)) +
  geom_pointrange(aes(ymin = Q2.5, 
                      ymax = Q97.5) + 
  geom_hline(yintercept = mean(a_df_exp2$a_mean), 
             size = 0.5, 
             col = "red") + 
    scale_y_continuous(expression(paste("exp(varying intercept), ", beta["0j"]))) + 
...
ggsave("fig_comm_intercepts.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 90, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
```
...
# Plot community-level beta zero's posterior mean and 95% credible interval
ggplot(data = Mglm1_df3, 
       aes(x = variable, 
           y = mean_exp)) +
  geom_pointrange(aes(ymin = Q2.5_exp, 
                      ymax = Q97.5_exp) + 
  scale_y_continuous(expression(paste("% change in gasoline consumption / hh"," ,", beta[]))) + 
  theme(axis.text.x = element_text(angle = 45, vjust=0.5, size=7), axis.title.x=element_blank(), axis.text.y = element_text(size=7), axis.title.y = element_text(size=7))+
  scale_x_discrete(labels = c("Fish harvest 1000lbs", "Game harvest 1000lbs","Snowmobiles","Boats","ATVs", "Cars & trucks"))

ggsave("fig_hh_coeff_interpret.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 90, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
```
...
modelDataPlot$FishLabel <- with(modelDataPlot,ifelse(FishSumEdible1000>1.3,commname,""))
...
GamePlot <- ggplot(data = modelDataPlot, aes(x=LargeGameSumEdible1000, y=an_gal_total, color=commname, label = GameLabel)) +
 geom_point()+
  ...
  xlab("Large game harvest in 1000s of lbs")+
  ylab("Annual gasoline consumption in gallons")+
...
FishPlot <- ggplot(data = modelDataPlot, aes(x=FishSumEdible1000, y=an_gal_total, color=commname, label = FishLabel)) +
 geom_point()+
  geom_text(hjust=1.01, vjust=.1, size=3) +
  xlab("Fish harvest in 1000s of lbs")+
  ylab("Annual gasoline consumption in gallons")+
