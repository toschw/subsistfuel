...
hh_data2$gal_log <- log(hh_data2$an_gal_total)

#Checking normal distribution fit
Cplot <- car::qqPlot(hh_data2$gal_log, "norm", xlab="normal quantiles", ylab="log(gasoline consumption)",envelope=0.95, id=FALSE)

#log-transforming income variable
hh_data2$hhincadj_log <-ifelse(hh_data2$hhincadj==0,0,log(hh_data2$hhincadj))
```
...
keep <- c("an_gal_total", "gal_log", "FishSumEdible1000", "LargeGameSumEdible1000", "atvs", "sms", "boats", "cars", "commname","HHSIZE","hhincadj_log")
modelData <- hh_data3[keep]
modelData <- modelData%>%
  na.omit()
```

#preparing modelData for archiving
```{r}
keepArchive <- c("an_gal_total","gal_log", "FishSumEdible1000", "LargeGameSumEdible1000", "atvs", "sms", "boats", "cars", "commname", "hhincadj_log", "HHSIZE")
archiveData <- hh_data3[keepArchive]
commIDset <- read.csv("D:/Dropbox/DATA/2011_Schwoerer_subsistence_fuel_survey/commID.csv",stringsAsFactor=FALSE)
archiveData <- archiveData%>%
  left_join(commIDset, by="commname")%>%
    na.omit()
archiveData <- archiveData[,-9]
colnames(archiveData)[colnames(archiveData)=="commID"] <- "commname"
write.csv(archiveData,"D:/Dropbox/DATA/2011_Schwoerer_subsistence_fuel_survey/archiveData.csv",row.names=FALSE)
```
...
library(ggplot2)
plot3 <- ggplot(plotData, aes(x=commname, y=probability, color=tertile, shape=commname, stroke=1)) +
...
ggsave("Figures/efficiency_probability.bmp", plot = last_plot(), device = "bmp", path = NULL,
...
usr <- par('usr')
p <- ggplot(data = sortData3[sortData3$idMax >5,], aes(x=idPerc, y=cumWeightPerc, color=eff2, shape=commname, stroke=1)) +
   scale_shape_manual(values=c(15,17,7,18)) + 
  ...
  theme_classic() 
p + geom_segment(aes(x = 70, y = 0, xend = 70, yend = 70))

ggsave("Figures/Fig2.bmp", plot = last_plot(), device = "bmp", path = NULL,
...
Model formulas: 
 ...
 Mglm3:  gal_log ~ 1 + FishSumEdible1000 + LargeGameSumEdible1000 + atvs + 
	   sms + boats + cars + HHSIZE + hhincadj_log + (1 | commname)      elpd_diff se_diff
Mglm1  0.0       0.0   
Mglm2 -1.1       2.6   
Mglm3 -1.4       2.4
...
ggsave("Figures/rhat_fig.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 90, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
ess_fig <- plot(Mglm1, "ess")
ggsave("Figures/ess_fig.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 90, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
```
...
ggsave("Figures/yyrepGrid_fig.bmp", plot = last_plot(), device = "bmp", path = NULL,
...
ggsave("Figures/PPC_fig.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 90, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
```
...
con_test_fig
ggsave("Figures/con_test_fig.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 90, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
```
...
pred_bands
ggsave("Figures/pred_bands_fig.bmp", plot = last_plot(), device = "bmp", path = NULL,
...
ggsave("Figures/betas.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 140, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
```
...
ggsave("Figures/Harvest_fig2.bmp", plot = last_plot(), device = "bmp", path = NULL,
...
Code from a previous figure showing harvest tertiles
...
ggsave("Figures/Harvest_and_efficiency.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 190, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
