...
modelData$total <- modelData$FishSumEdible1000 *1000 + modelDataPlot$LargeGameSumEdible1000 *1000
...
effSummary3 <-  effSummary2%>%
    mutate(
        lowest.PCT = round(lowest_tertile/sum(lowest_tertile), 3),
        middle.PCT = round(middle_tertile/sum(middle_tertile), 3),
        top.PCT = round(top_tertile/sum(top_tertile), 3))
rownames(effSummary3) <- c("higher_than_community_median","lower_than_community_median")


#for Tok solely
effSummaryTok <- sortData3[with(sortData3, commname=="Tok"),]%>%
  group_by(eff2,superhh2)%>%
  summarise(count = n())%>%
  spread(superhh2, count)

effSummaryTok <- column_to_rownames(effSummaryTok, var="eff2")%>%
  mutate(
        lowest.PCT = round(lowest_tertile/sum(lowest_tertile), 3),
        middle.PCT = round(middle_tertile/sum(middle_tertile), 3),
        top.PCT = round(top_tertile/sum(top_tertile), 3))
rownames(effSummaryTok) <- c("higher_than_community_median","lower_than_community_median")
```
